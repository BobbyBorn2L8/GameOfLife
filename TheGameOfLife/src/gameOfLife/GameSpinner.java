...
public class GameSpinner extends GameEntity{
				...
				if(degrees == 720)
				{
					...
					if (gameLogic.getCurrentPlayer().isStuck())
					{
						...
						if (result >= 5)
						{
							...
							gameLogic.getCurrentPlayer().movePlayerXTiles(gameLogic.getCurrentPlayer().getMovesRemaining(), 
									gameLogic.getBoard(), graphicsPanel);
	 ...
	 * @author Alex Crowley 40121793
	 */
	public int spinGameSpinner(boolean isMoving)
	{
		...
		if (isMoving)
		{
			if(spinResult == 10)
			{
				...
				Object[] options = {"OK"};
			    JOptionPane.showOptionDialog(null,  "Happy Birthday You Are Now " + gameLogic.getCurrentPlayer().getPlayerAge() 
			    		+ " Years Old!!", "HAPPY BIRTHDAY", JOptionPane.PLAIN_MESSAGE, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
		...
		GameLogic.gameLog.addLog("Spinner Result: " + result);
		return spinResult;
	}
	...
	public void randomiseStockMarketMultiplier()
	{
		...
		// JOptionPane.showConfirmDialog(null, "Stocks have changed, new stock multiplier is " + (int)(randomNumber * 100) + "%");
		Object[] options = {"OK"};
	    JOptionPane.showOptionDialog(null,  "Stocks have changed, new stock multiplier is " + (int)(randomNumber * 100) + "%", "Stock Market Change!", JOptionPane.PLAIN_MESSAGE, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	}
	...
	public void randomiseHousingMarketMultiplier(){
		...
		// JOptionPane.showConfirmDialog(null, "House prices have changed, new house multiplier is " + (int)(randomNumber * 100) + "%");
		Object[] options = {"OK"};
	    JOptionPane.showOptionDialog(null,  "House prices have changed, new house multiplier is " + (int)(randomNumber * 100) + "%", "Housing Market Change!", JOptionPane.PLAIN_MESSAGE, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	}
